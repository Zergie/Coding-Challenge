...
The demo can use less than 1 GB, but catalog scans and the version row add Table operations. Actual charges depend on usage and the subscription's offer, credits, taxes, and any network transfer. Check the subscription's pricing and the Azure pricing calculator before provisioning; monitor resource group Cost Analysis afterward.
